...
#
# Usage: query-batch.sh <batch_id> [org_num] [channel]   (org=3, channel=logchannel)
set -e
BATCH_ID="${1:?batch id required}"
N="${2:-3}"
CHANNEL="${3:-logchannel}"
...
echo ">>> QueryMerkleBatch($BATCH_ID) on $CHANNEL from Org${N}"
peer chaincode query -C "$CHANNEL" -n logchaincode \
