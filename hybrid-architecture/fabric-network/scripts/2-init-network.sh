...
if ! peer channel list 2>/dev/null | grep -q "$CHANNEL_NAME"; then
    ...
    echo "  ℹ️  peer0 is already in the channel"
fi
sleep 2
...
if ! peer channel list 2>/dev/null | grep -q "$CHANNEL_NAME"; then
    ...
    echo "  ℹ️  peer1 is already in the channel"
fi
sleep 2
...
if ! peer channel list 2>/dev/null | grep -q "$CHANNEL_NAME"; then
    ...
    echo "  ℹ️  peer2 is already in the channel"
fi
sleep 2
...
if peer lifecycle chaincode queryinstalled 2>/dev/null | grep -q "logchaincode_1"; then
    echo "ℹ️  Chaincode already installed, retrieving Package ID..."
...
# APPROVE CHAINCODE
# ============================================
echo "✔️  Checking chaincode approval..."
...
if peer lifecycle chaincode checkcommitreadiness \
    ...
    echo "ℹ️  Chaincode already approved by Org1MSP"
else
    echo "✍️  Approving chaincode for Org1MSP..."
